...
- **Bank holidays:** `server/bankholidays.js` — fetches gov.uk's free `bank-holidays.json` (England & Wales division only), refreshed daily at most and cached to `<data dir>/bank-holidays.json`. `getHolidays()` never throws; on failure it returns the last known dates with `stale: true`.
...
- **API:** `GET /api/budget`, `PUT /api/budget` (saves the entire state, debounced 500ms client-side), `GET /api/trading212` (`?refresh=1` bypasses the cache), `GET /api/bank-holidays` (`?refresh=1` bypasses the cache), `GET /api/backup`, `GET /api/backups`, `POST /api/restore`.
...
- Paydays: `person1PayDay` / `person2PayDay` hold a day-of-month or the string `'last'` (last working day of the month). `nextPayDate()` clamps to short months and rolls a non-working date **back** to the previous working day — the 25th on a Saturday pays on the 24th, and the last working day of August 2026 is the 28th because the 31st is the summer bank holiday. Non-working days are weekends plus the England & Wales bank holidays from `/api/bank-holidays`, passed in as a `Set` of `YYYY-MM-DD` strings; with an empty set it degrades to weekends only. Defaults are `'last'` for person1 and `25` for person2; the countdown and its editor live in the Income card on the Overview tab.
...
- **Mortgage** (`data.mortgage`, bottom of the Bills tab): `lender`, `balance`, `originalAmount`, `rate`, `payment`, `overpayment`, `startDate` and `rateExpiry` (`YYYY-MM-DD` strings, `''` when unset). **The monthly payment is the household bill whose name matches `/mortgage/i`** — one number, edited in one place; `mortgage.payment` is only the fallback when no such bill exists. Payments made = `monthsBetween(startDate, today)`, and `amortiseFor()` replays that many payments from `originalAmount` at the current rate to split them into interest and capital. **The balance left to pay is that replay's remainder**, not a stored figure — `mortgage.balance` is only used when `balanceManual` is true (set by typing over it) or when there isn't enough to derive from. A `↺ Use £x` chip drops the override and goes back to the calculated figure. The app stores no statement history, so that split is an estimate: a past rate change or an overpayment shows up as drift between the calculated balance and a manually entered one, which the card says out loud once the gap passes £500. `amortise(balance, annualRate, payment)` runs the balance down month by month (so interest follows the shrinking balance and an overpayment is just a bigger payment) and returns `null` when the payment doesn't cover the monthly interest, which the card reports rather than drawing a fake date. The overpayment calculator diffs two `amortise()` runs for the months and interest saved. `rateExpiry` turns amber within 90 days and when past.
...
- The uninvested-cash interest rate is not available from the API. It's stored as `t212CashRate` in `budget.json` and edited on the ISA card in the Savings tab — that whole uninvested-cash block is hidden when the free cash rounds to zero, and the rate then falls back to a compact one-line editor under the account total so it can still be set.
